...
class camera_thread(threading.Thread):#subclass of thread
    ...
    def run(self):
        ...
        debugging = False        #Set true if Debugging code should run
        ...
        if testing == False:
            ...
            while camera.isOpened():
                    ...
                    for r in range(0,height, step/3):
                        ...
                        for item in colorsCaught.itervalues():
                            if isColor(item, red, green, blue):
                                ...
                                item.averageX=(leftMostX+rightMostX)/2
                                item.averageY=(bottomMostY+topMostY)/2

                                if debugging:
                                    for x in range(item.averageX-2,item.averageX+2):
                                        for y in range(item.averageY-2, item.averageY+2):
                                            if y>0 and y<height and x>0 and x<width:
                                                frame[x,y]=(255,255,255)
                                    for x in range(item.averageX-1,item.averageX+1):
                                        for y in range(item.averageY-1, item.averageY+1):
                                            if y>0 and y<height and x>0 and x<width:
                                                frame[x,y]=(0,255,0)                
                            else:
                                frame[c,r]= (0,0,0) 
                ...
                for key, item in colorsCaught.iteritems():
                    coordinateY = float(item.averageY-(halfH))/(halfH)
                    coordinateX = float(-(item.averageX-(halfW))/(halfW))
                    self.colorDetectionList[key+'X']=coordinateY
                    self.colorDetectionList[key+'Y']=coordinateX
