...
public class TankControl : MonoBehaviour
{
	...
	public Dictionary<string, JSONNode> bullets;
	...
	public Vector3 getBulletPosition(string key){
		float X = 0.0f, Y = 0.0f, Z = 0.0f;
		Vector3 pos;
		...
		// Extract the (X,Y,Z) as a float and make it a new vector 3. 
		X = (infile["projectiles"][key]["orientation"]["linear"][0].AsFloat);
		Y = (infile["projectiles"][key]["orientation"]["linear"][1].AsFloat);
		Z = (infile["projectiles"][key]["orientation"]["linear"][2].AsFloat);
	...
	public Quaternion getBulletAngle(string key){
	...
	// Extract the (W,X,Y,Z) as a vector 4. 
		W = (infile["projectiles"][key]["orientation"]["angle"]["a"].AsFloat);
		X = (infile["projectiles"][key]["orientation"]["angle"]["i"].AsFloat);
		Y = (infile["projectiles"][key]["orientation"]["angle"]["j"].AsFloat);
		Z = (infile["projectiles"][key]["orientation"]["angle"]["k"].AsFloat);
	...
	public void firebullets(){
		...
		foreach(string key in bullets.Keys){
			bull[x].SetActive(true);
			bull[x].transform.position = getBulletPosition(key);
			bull[x].transform.rotation = getBulletAngle(key);
			x = x+1;
		}
	}
	...
	void Start () {
		...
		tcpIsPaused = false;
		locked = false;
		bull = new GameObject[10];
		for (int i = 0; i<10; i++) {
			bull[i] = Instantiate(Bullet);
			bull[i].SetActive(false);
		}
		...
		while (true) {
			...
			if (!tcpIsPaused){
				//if(locked == false){
					...
					bullets = new Dictionary<string, JSONNode>();
					...
					//threadInit = true;
					locked = true;}
					foreach (KeyValuePair<String, JSONNode> pair in infile["prefabs"].AsObject){
